...
Provider 接口本身不绑定具体操作系统；只要能访问 `manifest.json` 和 bundle，并调用目标模型服务，Provider 就可以被加载。桌面端运行能力仍受宿主系统限制：

- **macOS**：当前主要验证平台，屏幕录制、辅助功能等权限需要用户授权。
- **Windows**：支持 Provider 加载和本地运行；截图、窗口定位和输入自动化需要在目标 Windows 环境验证。
- **Linux**：Provider 加载逻辑可用，但端到端桌面自动化受 Wayland/X11、窗口管理器、截图权限和输入模拟能力影响，当前按实验性支持处理。
